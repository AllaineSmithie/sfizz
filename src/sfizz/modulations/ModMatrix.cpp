...
namespace sfz {
    ...
    void ModMatrix::beginCycle(unsigned numFrames, const bool avoidBufferReadyToFalsIterations)
    {
        Impl& impl = *impl_;

        impl.numFrames_ = numFrames;

        if (avoidBufferReadyToFalsIterations)
            return;
    ...
    void ModMatrix::endCycle(const bool avoidEndCycleGenerateDiscardedIterations)
    {
        ...
        if (avoidEndCycleGenerateDiscardedIterations)
            return;
