...
namespace sfz {
    ...
    void Synth::renderVoiceBlock(const int voice_id, AudioSpan<float> buffer) noexcept
    {
        ...
        mm.beginCycle(numFrames, true);

        NumericId<Voice> r_id(voice_id);
        { // Main render block
            for (auto& voice : impl.voiceManager_)
            {
                if (voice.isFree() || voice.getId() != r_id)
                    continue;
        ...
        /* { // Main render block
              for (auto& voice : impl.voiceManager_)
              {
                  ...
                  mm.setSamplesPerBlock(numFrames);
    ...
    void Synth::renderBlockEmpty(AudioSpan<float> buffer) noexcept
    {
        Impl& impl = *impl_;
        ScopedFTZ ftz;
        auto& callbackBreakdown = impl.callbackBreakdown_;
    ...
    void Synth::Impl::startVoice(Layer* layer, int delay, const TriggerEvent& triggerEvent, SisterVoiceRingBuilder& ring, int* r_id) noexcept
    {
        ...
        if (r_id)
            *r_id = selectedVoice->getId().number();
    ...
    int Synth::getVoicePosition(const int p_voice_id) const noexcept
    {
        Impl& impl = *impl_;
        NumericId<Voice> voice_id(p_voice_id);
        for (auto& voice : impl.voiceManager_)
        {
            if (voice.getId() == voice_id)
                return voice.getAge();
            /*const Region* region = voice.getRegion();
            if (!region)
                continue;
    ...
    bool Synth::getVoiceBlockCopy(const int p_voice_id, AudioSpan<float> buffer) noexcept
    {
        Impl& impl = *impl_;
        NumericId<Voice> voice_id(p_voice_id);
        for (auto& voice : impl.voiceManager_)
    ...
    void Synth::setVoicePosition(const int p_voice_id, const int numframes) noexcept
    {
        Impl& impl = *impl_;
        NumericId<Voice> voice_id(p_voice_id);
        for (auto& voice : impl.voiceManager_)
    ...
    int Synth::playRegionByID(const int id) noexcept
    {
        SisterVoiceRingBuilder ring;
        Impl& impl = *impl_;
        ...
        for (std::unique_ptr<Layer>& layer : impl.layers_)
        {
            ...
            if (region.id == _id)
            {
                ...
                impl.startVoice(layer.get(), 0, triggerEvent, ring, &r_voice_id);
                return r_voice_id;
            }
        }
        return r_voice_id;
